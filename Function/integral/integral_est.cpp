...
)
{
    ...
    NumericVector beta(p), res(n), Covbeta(n), int_cexpbetaD(n), int_cdexpbetaD_1(n), int_cdexpbetaD_2(n), fn(p+1), SY(k), dSY(k), new_parameters(p+1);
    NumericMatrix int_D(n, k), dNt(n, k), Yt(n, k), int_expbetaD(n, k), int_Lam(n, k), int_dexpbetaD(n, k), int_dexpbetaD_Lam(n, k), int_expbetaD_dLam(n, k), int_expbetaD_dLam_dalpha(n, p), dPhi(n, p+1), Hessian(p+1, p+1), inv_Hessian(p+1, p+1);
        ...
        for (int i = 0; i < n; i++)
        {
                ...
                if (Yt(i, j) > 0)
                {
                    if (j == 0)
                    {
                        ...
                        { 
                            ...
                            int_dexpbetaD = int_D(i, j) * int_D(i, j) / 2;
                        }
                    }
                    else
                    {
                    ...
                    for (int ii = 0; ii < n; ii++)
                    {
                        ...
                        if (j == 0)
                        {
                            ...
                            if (betaD != 0)
                            {
                                ...
                                int_cdexpbetaD_1[ii] = (IV[i] > 0.5) ? (int_D(i, j) * tmp_cexpbetaD/((tmp_cD)*betaD) - int_cexpbetaD[ii]/((tmp_cD)*betaD)):int_D(i, j) * int_cexpbetaD[ii];
                                int_cdexpbetaD_2[ii] = (IV[ii] > 0.5) ? (int_D(ii, j) * tmp_cexpbetaD/((tmp_cD)*betaD) - int_cexpbetaD[ii]/((tmp_cD)*betaD)):int_D(ii, j) * int_cexpbetaD[ii];
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            if (betaD != 0)
                            {
                                ...
                                int_cdexpbetaD_1[ii] = (D_status(i, j-1) > 0.5) ? 
                                                    ...
                                                    int_cexpbetaD[ii]/((tmp_cD)*betaD)):int_D(i, j) * int_cexpbetaD[ii];
                                int_cdexpbetaD_2[ii] = (D_status(ii, j-1) > 0.5) ? 
                                                    ((int_D(ii, j) * tmp_cexpbetaD - int_D(ii, j-1) * old_cexpbetaD)/((tmp_cD)*betaD) - 
                                                    int_cexpbetaD[ii]/((tmp_cD)*betaD)):int_D(ii, j) * int_cexpbetaD[ii];
                            }
                            else
                            {
                        ...
                        int_expbetaD_dLam(i, j) = int_expbetaD_dLam(i, j) + (dNt(ii, j) * int_D(ii, j) * tmp_cexpbetaD - 
                                                        ...
                                                        tmp_cexpbetaD * Yt(ii, j) * D_status(ii, j)) * SY[j] - tmp * dSY[j];
                        for (int kk = 0; kk < p; kk++)
                        {
                ...
                for (int kk = 0; kk < p+1; kk++)
                {
                    if (kk == 0)
                    {
                        dPhi(i, kk) = dPhi(i, kk) + dNt(i, j) * int_D(i, j) * exp(betaD * int_D(i, j)) - 
                                        ...
                                        Yt(i, j) * exp(int_D(i, j) * betaD) * D_status(i, j) - 
                                        Yt(i, j) * int_expbetaD_dLam(i, j);
                    }
                    else
                    {
                        dPhi(i, kk) = dPhi(i, kk) - Yt(i, j) * Covariates(i, kk-1) * exp(betaD * int_D(i, j)) - 
            ...
            for (int kk = 0; kk < p+1; kk++)
            {
                ...
                else
                {
                    ...
                    for (int kkk = 0; kkk < p+1; kkk++)
                    {
                        Hessian(kk, kkk) = Hessian(kk, kkk) + Covariates(i, kkk-1) * dPhi(i, kkk);
                    }
                }
            }
        }
        inv_Hessian = arma::inv(Hessian);
    }
