...
for(i in 1:nrep){
  tmpdata = simdata(n = n)
  s1 = nleqslv(c(0.1,0.075,0.075), ConstantF, time = tmpdata$time,
              event = tmpdata$event, IV = tmpdata$IV,
              Covariates = cbind(tmpdata$Covariates, tmpdata$U),
              D_status = tmpdata$D_status, stime = tmpdata$stime)
  s2 = nleqslv(c(0.1,0.075), ConstantF, time = tmpdata$time,
               event = tmpdata$event, IV = tmpdata$IV,
               Covariates = tmpdata$Covariates,
               D_status = tmpdata$D_status, stime = tmpdata$stime)
